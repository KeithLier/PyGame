...
def check_event(setting, screen, ship, bullets, aliens, stats, scoreboard, button):
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_play_button(setting, screen, ship, bullets, aliens, stats, scoreboard, button, mouse_x, mouse_y)

def update_screen(setting, screen, stats, scoreboard, ship, aliens, bullets, button):
...
def update_bullets(setting, screen, stats, scoreboard, ship, aliens,bullets):
    bullets.update()
    for bullet in bullets.copy():
        if bullet.rect.bottom <= 0:
            bullets.remove(bullet)
    chect_bullet_alien_collisions(setting, screen, stats, scoreboard, ship, aliens, bullets)
...
def get_number_aliens_x(setting, alien_width):
    available_space = setting.screen_width - alien_width
    number_aliens_x = int(available_space / (2 * alien_width))
    return number_aliens_x

def get_number_rows(setting, ship_height, alien_height):
    available_space = (setting.screen_height - (3 * alien_height) - ship_height)
...
def check_play_button(setting, screen, ship, bullets, aliens, stats, scoreboard, button, mouse_x, mouse_y):
...
def chect_bullet_alien_collisions(setting, screen, stats, scoreboard, ship, aliens,bullets):
...
def update_aliens(setting, screen, stats, scoreboard, ship, aliens, bullets):

    check_fleet_edges(setting, aliens)
    aliens.update()

    if pygame.sprite.spritecollideany(ship, aliens):
        ship_hit(setting, screen, stats, scoreboard, ship, aliens, bullets)

    check_aliens_bottom(setting, screen, stats, scoreboard, ship, aliens, bullets)
...
def ship_hit(setting, screen, stats, scoreboard, ship, aliens, bullets):
...
def check_aliens_bottom(setting, screen, stats, scoreboard, ship, aliens,
        bullets):
